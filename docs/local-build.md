...
> | `--update` | — | 不加则**不动**已有源码树，避免覆盖你本地的调试修改 |
...
`build.sh` 默认**不动**已有源码树，只打印当前 HEAD；要更新得显式加 `--update`。这样做是因为你在这棵树里的主要活动是改内核补丁和 DTS，基线在不知情时变动会让「改动前后对比」失去意义 —— 否证一个假设的前提是只有一个变量在动。
...
| 已分叉，但本地有未提交修改或自己的提交 | **停下**并列出来，交给你 stash 或建分支，绝不静默丢弃 |
