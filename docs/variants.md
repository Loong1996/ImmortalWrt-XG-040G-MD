...
>
> `206-airoha-an7581-probe-dram-size.patch` 补上真正的探测：用地址回绕（颗粒比所探地址小的时候，高地址的写会绕回低地址锚点）。锚点放在 ATF 保留区之上并在返回前还原，可在重定位前安全运行。

| 变体 | 引导程序 | 自适应 |
| --- | --- | --- |
| `tcboot` | 第三方 U-Boot 2025.01 | ✅ 实测有效（自带探测） |
| `ubi` | OpenWrt 自建 U-Boot | ✅ 打了 `206` 之后有效；**之前恒为 512M** |
| `stock` | 原厂引导程序 | 未验证，换过颗粒建议手动指定 |

`tcboot` 变体实测（1G 颗粒，刷的是 **512M 档**编出来的固件）：
...
> **上游把这个属性写死成 `0x1fe00000`（512M − 2M）**，`master-XG-040G-MD` 线已经改成 `0x7fe00000`（2G，等于不裁剪）。
>
